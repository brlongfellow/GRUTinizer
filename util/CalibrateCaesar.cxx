...
#define TOTAL_PEAKS_TO_FIT 12 // Maximum number of peaks to find for each detector
#define FIT_PADDING 25    // Distance to go to left and right of peak for fitting with gaussian
...
//      this->known_energies[5][1] = 0;//keep failing to find this peak
      ...
      this->known_energies[6][1] = 4438; //AmBe (9Be(a,n)12C*)y
    ...
    void fillKnownRanges(){ //Low/High X-Axis Values for Ease of Finding Peaks
      ...
      known_ranges[5][0]  = 310;
      known_ranges[5][1]  = 2000;

      known_ranges[6][0] = 975;
      known_ranges[6][1] = 2000;
    }
};
...
void CalibrateCaesar(char *in_hist_file_name, char *in_source_list, char *out_cal_file_name, 
  ...
  for (unsigned int ring = 0; ring < N_RINGS; ring++){
    for (unsigned int det = 0; det < det_per_ring[ring]; det++){
	  ...
	  extra_find_padding = 200; //give ambe fit extra find padding for estimate of correct energy
          extra_fit_padding = 0;
        }
        else if (source_list[source].name == "ba133") {
          extra_rebinning_factor = 0;
          extra_find_padding = 0;
          extra_fit_padding = -20;
        }
        else {
          extra_rebinning_factor = 0;
          extra_find_padding = 0;
