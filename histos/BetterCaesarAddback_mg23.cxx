  ...
  if(caesar && s800) {
    ...
    for(unsigned int y=0;y<caesar->Size();y++) {
      ...
      if (hit.IsValid()){//only accept hits with both times and energies
       ...
       if (newmg23blob->IsInside(objtac_corr,ic_sum)){
          if(timingcut_mg23->IsInside(corr_time,hit.GetDoppler(beta,z_shift,&track))){
      ...
      }//hit is valid and is not an overflow
    ...
    for (int y = 0; y < num_addback_hits; y++){
      ...
      if (hit.IsValid()){//only accept hits with both times and energies
        ...
        if (newmg23blob->IsInside(objtac_corr,ic_sum)){
          if (timingcut_mg23->IsInside(corr_time, energy_dc)){
            if (InBeam_Mid->IsInside(objtac,xfptac)){
