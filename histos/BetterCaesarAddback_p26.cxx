...
void MakeHistograms(TRuntimeObjects& obj) {
  ...
  if(!si25blobtop) {
     TPreserveGDirectory Preserve;
     TFile fcut("/mnt/analysis/pecan-2015/longfellow/e10002/si25blobtop.root");
  ...
  if(caesar && s800) {
    ...
    for(unsigned int y=0;y<caesar->Size();y++) {
      ...
      if (hit.IsValid()){//only accept hits with both times and energies
       ...
       if (p26blobtop->IsInside(objtac_corr,ic_sum)){
          if(timingcut_p26->IsInside(corr_time,hit.GetDoppler(beta,z_shift,&track))){
    ...
    for (int y = 0; y < num_addback_hits; y++){
      ...
      if (hit.IsValid()){//only accept hits with both times and energies
        ...
        if (p26blobtop->IsInside(objtac_corr,ic_sum)){
          if (timingcut_p26->IsInside(corr_time, energy_dc)){
            if (InBeam_Top->IsInside(objtac,xfptac)){
    ...
    for (unsigned int i = 0; i < num_hits_singles; i++){
      for (unsigned int j = i+1; j < num_hits_singles; j++){
        ...
        obj.FillHistogram(dirname, histname,
                          4096,0,4096, energies_singles.at(i),
                          4096,0,4096, energies_singles.at(j));
        obj.FillHistogram(dirname, histname,
                          4096,0,4096, energies_singles.at(j),
                          4096,0,4096, energies_singles.at(i));
        histname = "poss_diff";
    ...
    if (addback_mult == 1){
      ...
      for (int i = 0; i < addback_mult; i++){
        for (int j = i+1; j < addback_mult; j++){
          ...
          obj.FillHistogram(dirname, histname,
              250,0,4096, energies_addback.at(j),
              256,0,4096, energies_addback.at(i));
