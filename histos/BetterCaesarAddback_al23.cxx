  ...
  if(caesar && s800) {
    ...
    for(unsigned int y=0;y<caesar->Size();y++) {
      ...
      if (hit.IsValid()){//only accept hits with both times and energies
       ...
       if (newal23blob->IsInside(objtac_corr,ic_sum)){
          if(timingcut_al23->IsInside(corr_time,hit.GetDoppler(beta,z_shift,&track))){
    ...
    for (int y = 0; y < num_addback_hits; y++){
      ...
      if (hit.IsValid()){//only accept hits with both times and energies
        ...
        if (newal23blob->IsInside(objtac_corr,ic_sum)){
          if (timingcut_al23->IsInside(corr_time, energy_dc)){
            if (InBeam_btwnTopMid->IsInside(objtac,xfptac)){
