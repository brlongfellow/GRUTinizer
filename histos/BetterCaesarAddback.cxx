...
bool OutgoingBeam(TRuntimeObjects& obj,GCutG *incoming) {
  ...
  //if(caesar){
    //if(caesar->GetCaesarHit(0).GetEnergy()>500){
                //histname = "IC_vs_OBJTOF_PID_firstElab>500";
                //obj.FillHistogram(dirname,histname,
  ...
  histname = "AFP_vs_CRDC1X";
  obj.FillHistogram(dirname,histname,
        	    500,-500,500,s800->GetCrdc(0).GetDispersiveX(),
        	    1000,-1,1,afp);

  histname = "AFP_vs_CRDC2X";
  obj.FillHistogram(dirname,histname,
        	    500,-500,500,s800->GetCrdc(1).GetDispersiveX(),
        	    1000,-1,1,afp);
  ...
  for(unsigned int i=0; i<s800->GetHodoscope().Size(); i++){
    ...
    histname = "Hodoscope_HitCheck";
    obj.FillHistogram(dirname,histname,50,0,50,s800->GetHodoscope().GetHodoHit(i).GetChannel(),
                                       10,-5,5,s800->GetHodoscope().GetHodoHit(i).GetHitCheck());
...
int HandleCaesar(TRuntimeObjects& obj,GCutG *incoming,GCutG *outgoing) {
  ...
  for(unsigned int y=0;y<caesar->Size();y++) {
      ...
      if (hit.IsValid()){//only accept hits with both times and energies
          ...
          obj.FillHistogram(dirname,"corrtime_vs_energy",2000,-2000,2000,corr_time,
                                                         1024,0,8192,hit.GetEnergy());

          obj.FillHistogram(dirname,"scatterangle_vs_doppler",1000,0,1,s800->Track().Theta(),
                                                         1024,0,8192,energy_dc);

          obj.FillHistogram(dirname,"azita_vs_doppler",1000,0,7,s800->Azita(),
                                                         1024,0,8192,energy_dc);

          obj.FillHistogram(dirname,"ata_vs_doppler",1000,-0.2,0.2,s800->GetAta(),
                                                         1024,0,8192,energy_dc);

          obj.FillHistogram(dirname,"bta_vs_doppler",1000,-0.2,0.2,s800->GetBta(),
                                                         1024,0,8192,energy_dc);

          obj.FillHistogram(dirname,"scatterangle_vs_ata",1000,0,1,s800->Track().Theta(),
                                                         1000,-0.2,0.2,s800->GetAta());

          obj.FillHistogram(dirname,"scatterangle_vs_bta",1000,0,1,s800->Track().Theta(),
                                                         1000,-0.2,0.2,s800->GetBta());

          if(energy_dc>1229 && energy_dc<1429){
            obj.FillHistogram(dirname,"scatterangle_vs_ata_1329keV",1000,0,1,s800->Track().Theta(),
                                                         1000,-0.2,0.2,s800->GetAta());
            obj.FillHistogram(dirname,"scatterangle_vs_bta_1329keV",1000,0,1,s800->Track().Theta(),
                                                         1000,-0.2,0.2,s800->GetBta());
          }
  ...
  if (num_hits_singles == 1){
      ...
      obj.FillHistogram(dirname,"DTA_energy_dc_mult_one",
			        512,-0.2,0.2,s800->GetDta(),
                                1024,0,8192,energies_singles.at(0));
      for(unsigned int i=0; i<s800->GetHodoscope().Size(); i++){
        histname = "Hodoscope_CAESAR_mult_one";
        obj.FillHistogram(dirname,histname,1024,0,8192,energies_singles.at(0),
                                           1024,0,8192,s800->GetHodoscope().GetHodoHit(i).GetEnergy());
      }
  }
  ...
  if (addback_mult == 1){
      ...
      for(unsigned int i=0; i<s800->GetHodoscope().Size(); i++){
        histname = "Hodoscope_CAESAR_mult_one";
        obj.FillHistogram(dirname,histname,1024,0,8192,energies_addback.at(0),
                                           1024,0,8192,s800->GetHodoscope().GetHodoHit(i).GetEnergy());
      }
      if (n0_mult ==1){
  ...
  for (int i = 0; i < addback_mult; i++){
      ...
      for(unsigned int k=0; k<s800->GetHodoscope().Size(); k++){
        histname = "Hodoscope_CAESAR_mult_any";
        obj.FillHistogram(dirname,histname,1024,0,8192,energies_addback.at(i),
                                           1024,0,8192,s800->GetHodoscope().GetHodoHit(k).GetEnergy());
      }
  ...
  if (addback_mult_te == 1){
      ...
      for(unsigned int i=0; i<s800->GetHodoscope().Size(); i++){
        histname = "Hodoscope_CAESAR_mult_one";
        obj.FillHistogram(dirname,histname,1024,0,8192,energies_addback_te.at(0),
                                           1024,0,8192,s800->GetHodoscope().GetHodoHit(i).GetEnergy());
      }
      if (n0_mult_te ==1){
        histname = "ab_energy_dc_mult_one_n0";
