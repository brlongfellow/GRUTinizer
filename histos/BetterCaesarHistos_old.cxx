...
void MakeHistograms(TRuntimeObjects& obj) {
  ...
  static TCutG *mg23blob = 0;
  if(!mg23blob) {
    TPreserveGDirectory Preserve;
    TFile fcut("/mnt/analysis/pecan-2015/longfellow/e10002/mg23blob.root");
    mg23blob = (TCutG*)fcut.Get("mg23");
  }
  static TCutG *mg22blob = 0;
  if(!mg22blob) {
    TPreserveGDirectory Preserve;
    TFile fcut("/mnt/analysis/pecan-2015/longfellow/e10002/mg22blob.root");
    mg22blob = (TCutG*)fcut.Get("mg22");
  }
	...
	double xfp_focalplane = s800->GetXFP(0);
	  ...
	  if(InBeam_Mid->IsInside(objtac,xfptac)){
		...
		obj.FillHistogram("GATED",histname,
          ...
          if(mg23blob->IsInside(objtac_corr,ic_sum) && InBeam_Mid->IsInside(objtac,xfptac)){

	    histname = "Gamma_Gated_mg23blob";
	    obj.FillHistogram("GATED",histname,
			      1024,0,8192,hit.GetDoppler(beta,z_shift,&track));

            histname = "FindBeta_mg23blob";
	      for(int beta_i=0;beta_i<300;beta_i++){
		double beta_use = 0.2+(0.3/300.0)*double(beta_i);
		obj.FillHistogram("GATED",histname,
				  300,0.2,0.499,beta_use,
	  ...
	  }//end if inside mg23blob and inside mid incomingPID

          if(mg22blob->IsInside(objtac_corr,ic_sum) && InBeam_Mid->IsInside(objtac,xfptac)){
	    histname = "Gamma_Gated_mg22blob";
	    obj.FillHistogram("GATED",histname,
			      1024,0,8192,hit.GetDoppler(beta,z_shift,&track));

            histname = "FindBeta_mg22blob";
	      for(int beta_i=0;beta_i<300;beta_i++){
		double beta_use = 0.2+(0.3/300.0)*double(beta_i);
		obj.FillHistogram("GATED",histname,
				  300,0.2,0.499,beta_use,
	  ...
	  }//end if inside mg22blob and inside mid incomingPID
